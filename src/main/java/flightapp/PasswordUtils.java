...
public class PasswordUtils {
  ...
  public static byte[] saltAndHashPassword(String password) {
    ...
    byte[] combine = new byte[salt.length + saltedHash.length];
    System.arraycopy(salt, 0, combine, 0, salt.length);
    System.arraycopy(saltedHash, 0, combine, salt.length, saltedHash.length);
  ...
  public static boolean plaintextMatchesSaltedHash(String plaintext, byte[] saltedHashed) {
    ...
    byte[] users = hashWithSalt(plaintext, salt);
    for (int i = salt.length; i < saltedHashed.length; i++) {
      if (saltedHashed[i] != users[i - salt.length]) {
        return false;
      }
    }
    return true;
